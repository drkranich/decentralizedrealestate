...
export function StatCard({
  ...
  const accentClass =
    accent === "emerald" ? "bg-emerald/15 text-emerald" : "bg-skyblue/15 text-skyblue";
  return (
    <div
    ...
    >
      ...
      <div className="relative">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`flex h-9 w-9 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110 ${accentClass}`}
          >
...
export function Card({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={`animate-fade-in-up group relative overflow-hidden rounded-3xl border border-glass-border bg-card/60 p-6 shadow-soft backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-emerald/20 hover:shadow-elegant ${className}`}
    >
...
export function DemoDataBadge() {
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-emerald/30 bg-emerald/10 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-emerald">
      Dados reais
    </span>
  );
}
...
export function Badge({
  ...
  children: ReactNode;
  variant?: "default" | "emerald" | "blue" | "warn" | "muted";
}) {
  const map = {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border border-glass-border px-2.5 py-0.5 text-xs font-medium transition-colors ${map[variant]}`}
    >
      {children}
    </span>
  );
