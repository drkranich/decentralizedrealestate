...
function PropertyDetails() {
  ...
  const photoUrls = photos.map(
    (p) => supabase.storage.from("property-media").getPublicUrl(p.storage_path).data.publicUrl,
  );
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
            ...
            {property.price != null && (
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  {property.listing_type === "venda" ? "preço de venda" : "/ mês"}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="mt-6 flex gap-6 overflow-x-auto border-b border-glass-border">
            {["overview", "analytics", "documents", "maintenance", "smart devices", "bookings"].map(
              (t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
          ...
          <div className="mt-6 space-y-6">
            ...
            {tab === "analytics" && (
              <>
                ...
                <Card>
                  <p className="text-sm text-muted-foreground">
                    Análise de receita real depende de reservas e pagamentos processados pela
                    plataforma — ainda não existem transações reais para este imóvel, então nenhum
                    gráfico é mostrado aqui.
                  </p>
                </Card>
              </>
            )}
            ...
            {tab === "documents" && (
              <Card>
                ...
                <p className="text-sm text-muted-foreground">
                  Nenhum documento real anexado ainda. O upload de contratos ainda não está
            ...
            {tab === "maintenance" && (
              <Card>
                ...
                <p className="text-sm text-muted-foreground">
                  Ainda não existe uma tabela de manutenção real — nada foi registrado para este
                  imóvel.
                </p>
              </Card>
            )}
            ...
            {tab === "smart devices" && (
              <Card>
                ...
                <div className="grid gap-3 sm:grid-cols-2 opacity-60">
                  {[
                    { i: Lock, l: "Front door" },
                    { i: Thermometer, l: "Climate" },
                    { i: Lightbulb, l: "Living lights" },
                    { i: Camera, l: "Entry camera" },
                  ].map((d) => (
                    <div
                      key={d.l}
                      className="rounded-2xl border border-glass-border bg-secondary/30 p-4"
                    >
                      <div className="flex items-center gap-2">
                        <d.i className="h-4 w-4 text-muted-foreground" />
            ...
            {tab === "bookings" && (
              <Card>
                ...
                <p className="text-sm text-muted-foreground">
                  Ainda não existe um sistema de reservas real conectado — nenhuma reserva de fato
                  foi feita para este imóvel.
                </p>
              </Card>
            )}
          </div>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <div className="text-xs text-muted-foreground">
              Sem histórico de pagamentos ainda — ROI e ocupação aparecerão aqui quando houver
              reservas reais.
            </div>
          </Card>
            ...
            <div className="space-y-2 text-xs opacity-70">
              <div className="rounded-xl border border-glass-border bg-secondary/30 p-3">
                <div className="font-semibold flex items-center gap-1">
                  <Sparkles className="h-3 w-3" /> Sem dados suficientes
                </div>
                <div className="mt-1 text-muted-foreground">
                  Insights de IA precisam de histórico real de preços e ocupação.
                </div>
              </div>
            </div>
          </Card>
