...
type Role = "tenant" | "owner" | "investor" | "service_provider";
      ...
      <div className="flex min-h-screen items-center justify-center bg-secondary/30 px-4">
        <div className="w-full max-w-sm rounded-3xl border border-glass-border bg-card/70 p-8 text-center shadow-elegant backdrop-blur-2xl">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Enviamos um link de confirmação para <strong>{email}</strong>. Abra-o para ativar sua
            conta e depois volte para fazer login.
          </p>
          <Link
            to="/login"
            className="mt-6 inline-block rounded-full bg-emerald px-5 py-2 text-sm font-semibold text-white"
          >
            Ir para o login
          </Link>
        </div>
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-secondary/30 px-4 py-12">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-3xl border border-glass-border bg-card/70 p-8 shadow-elegant backdrop-blur-2xl">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Proprietário, inquilino, investidor ou prestador: entre no ecossistema certo.
        </p>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["tenant", "owner", "investor", "service_provider"] as Role[]).map((r) => (
                <button
                  key={r}
                  type="button"
                  onClick={() => setRole(r)}
                  className={`rounded-full px-3.5 py-2 text-sm font-medium transition-colors ${role === r ? "bg-foreground text-background" : "border border-glass-border bg-secondary/40"}`}
                >
                  {r === "tenant"
                    ? "Inquilino"
                    : r === "owner"
